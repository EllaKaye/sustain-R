...
```{r}
#| echo: false
ggplot(gender_perc, aes(x = fct_reorder(gender, -perc), y = perc)) +
...
```{r}
#| echo: false
ggplot(loc_perc, aes(x = fct_reorder(location2, -perc), y = perc)) +
  geom_col(fill = aubergine) +
  geom_text(aes(label = paste0(perc, '%')), vjust = -0.45, size = 5) +
...
- Bug BBQ - July 20th online (TBC)
- C book club - from May 16th

::: {.notes}
Recap of events mentioned above as final call to action
Bug NNQ may be hybrid with the in-person part in Basel
:::
